...
- Versions must stay synchronized in `package.json`, `src-tauri/tauri.conf.json`, and `src-tauri/Cargo.toml`. Use `bun run release --patch|--minor|--major` rather than editing them independently. The release command must preserve the protected `main` rules: it creates and validates a release PR, squash-merges it after required checks, and tags the resulting `main` commit instead of pushing a version commit directly.
